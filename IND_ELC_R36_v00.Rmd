...
# GIS info, map
```{r}
load("data/IND_adm1.RData")
load("data/model_regs.RData")

# (reg_names <- unique(as.character(IND@data$region)))
(reg_names <- unique(as.character(model_regs)))
(reg_names_in_gis <- as.character(IND@data$region))

(nreg <- length(reg_names))
(nreg_in_gis <- length(IND@data$region))

gis_ii <- IND$region %in% reg_names

plot(IND[gis_ii,])

# Neighbor regions
# if (!any((installed.packages())[, "Package"] == "spdep")) install.packages("spdep")
# nbr <- spdep::poly2nb(IND)
nbr <- spdep::poly2nb(IND[gis_ii,])
names(nbr) <- IND@data$region[gis_ii]

# Centers of the regions
reg_centers <- getCenters(IND[gis_ii,])
reg_area <- tibble(
  region = reg_centers$region,
  area_km2 = raster::area(IND[gis_ii,]) / 1e6
)
...
# Scenario 2: Exogenous grid 100GW between neighbours
```{r scen_RENBAL2, eval=FALSE}
...
scen_RENBAL2 <- interpolate(
  add(mdl, UHV, TRBD_UHV_NEI_FX_CAP, ELC2UHV, UHV2ELC, DEM_ELC_DH), 
...
# Scenario 3: Endogenous grid
```{r scen_RENBAL3, eval=FALSE}
# (optional) Scenario info:
{scen_descr <- "
...
scen_RENBAL3 <- interpolate(
  add(mdl, UHV, TRBD_UHV_NEI, ELC2UHV, UHV2ELC, DEM_ELC_DH), 
...
# if (!exists("scen_RENBAL3")) load("tmp/scen_RENBAL3.RData")
# if (!exists("scen_RENBAL4")) load("tmp/scen_RENBAL4.RData")
if (!exists("scen_RENBAL5")) load("tmp/scen_RENBAL5.RData")
# scen <- scen_RENBAL1
# scen <- scen_RENBAL2
# scen <- scen_RENBAL3
# scen <- scen_RENBAL4
